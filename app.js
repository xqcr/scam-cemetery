...
const characterImagePaths = {
  antiphisher: "antiphisher-v5.webp",
  pause: "pause-master.webp",
  qr: "qr-inspector.webp",
  oauth: "oauth-guard.webp",
  mfa: "mfa-sentry.webp",
  support: "support-inspector.webp",
  ai: "ai-auditor.webp",
  spearphishing: "spearphishing-detective-v5.webp",
};
const characterImageLoads = new Map();
let selected = null;
let busy = false;
let scrollAnimationFrame = null;

function preloadCharacterImage(card) {
  const src = `./assets/heroes-v4/${characterImagePaths[card.id]}`;
  if (!characterImageLoads.has(src)) {
    characterImageLoads.set(src, new Promise((resolve) => {
  ...
  return characterImageLoads.get(src);
}
...
async function reveal() {
  ...
  await Promise.all([
    preloadCharacterImage(personas[selected]),
    wait(reducedMotion ? 0 : 650),
  ]);
